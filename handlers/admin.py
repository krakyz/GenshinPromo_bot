...
async def confirm_expire_code(callback: CallbackQuery, bot: Bot):
    ...
    try:
        # ВАЖНО: Обновляем сообщения ПЕРЕД удалением кода из БД
        ...
        if success:
            await callback.message.edit_text(
                f"""✅ **Код успешно деактивирован!**

🗑️ **Код:** `{code}`
🔄 **Старые сообщения:** Обновлены
📊 **Статус:** Полностью удален из базы данных""",
            ...
            await callback.message.edit_text(
                f"❌ **Ошибка деактивации!**\n\nКод `{code}` не найден в базе данных.",
                parse_mode="HTML",
                reply_markup=get_admin_back_keyboard()
            )
        ...
        await callback.message.edit_text(
            f"❌ **Критическая ошибка!**\n\nНе удалось деактивировать код `{code}`.",
            parse_mode="HTML",
            reply_markup=get_admin_back_keyboard()
        )
...
# ДОПОЛНИТЕЛЬНЫЙ МЕТОД ДЛЯ РУЧНОЙ ПРОВЕРКИ ОБНОВЛЕНИЙ
@router.callback_query(F.data == "admin_check_expired", AdminFilter())
async def manual_check_expired(callback: CallbackQuery, bot: Bot):
    """Ручная проверка и обновление истекших кодов"""
    try:
        await callback.message.edit_text(
            "🔄 **Проверка истекших кодов...**\n\nЭто может занять некоторое время.",
            parse_mode="HTML"
        )
        
        from utils.broadcast import process_expired_codes
        processed_count = await process_expired_codes(bot)
        
        if processed_count > 0:
            result_text = f"""✅ **Проверка завершена!**

🗑️ **Обработано истекших кодов:** {processed_count}
🔄 **Все связанные сообщения обновлены**

Теперь пользователи видят актуальную информацию о кодах."""
        else:
            result_text = """ℹ️ **Проверка завершена!**

✅ **Истекших кодов не найдено**

Все активные коды в порядке."""
        ...
        logger.error(f"Ошибка ручной проверки истекших кодов: {e}")
        await callback.message.edit_text(
            "❌ **Ошибка при проверке кодов!**\n\nПопробуйте еще раз или обратитесь к разработчику.",
            parse_mode="HTML",
            reply_markup=get_admin_back_keyboard()
        )
...
# ОБНОВЛЕННОЕ ГЛАВНОЕ МЕНЮ АДМИНКИ (добавить кнопку проверки)
def get_admin_keyboard() -> InlineKeyboardMarkup:
    """Главное меню админ-панели с кнопкой проверки истекших кодов"""
...
# ДОПОЛНИТЕЛЬНАЯ ФУНКЦИЯ ДЛЯ ТЕСТИРОВАНИЯ СИСТЕМЫ ОБНОВЛЕНИЙ
@router.message(Command("test_updates"), AdminFilter())
async def test_message_updates(message: Message, bot: Bot):
    """Тестовая команда для проверки системы обновления сообщений"""
    try:
        ...
        if not codes:
            await message.answer("❌ Нет активных кодов для тестирования")
            return
        
        # Берем первый код для теста
        test_code = codes[0]
        
        await message.answer(f"""🧪 **Тестирование системы обновлений**

📋 **Выбран код для теста:** `{test_code.code}`
🔄 **Начинаю проверку связанных сообщений...**""")
        
        # Получаем сообщения связанные с кодом
        messages = await db.get_code_messages_by_value(test_code.code)
        
        await message.answer(f"""📊 **Результат проверки:**

🔗 **Найдено связанных сообщений:** {len(messages)}
📨 **Система отслеживания:** {'✅ Работает' if messages else '⚠️ Нет данных'}

💡 *Это означает, что при истечении кода {len(messages)} сообщений будут обновлены*""")
        
    except Exception as e:
        logger.error(f"Ошибка тестирования обновлений: {e}")
        await message.answer("❌ **Ошибка тестирования!**\n\nПроверьте логи бота.")

# ФУНКЦИЯ ДЛЯ ВОССТАНОВЛЕНИЯ ПРОПУЩЕННЫХ СВЯЗЕЙ
@router.callback_query(F.data == "admin_repair_links", AdminFilter())
async def repair_message_links(callback: CallbackQuery):
    """Восстанавливает пропущенные связи между кодами и сообщениями"""
    try:
        await callback.message.edit_text(
            """🔧 **Восстановление связей**

⚠️ **Внимание!** Эта функция попытается восстановить связи между активными кодами и сообщениями пользователей.

Это может занять время и не гарантирует 100% результат, так как старые сообщения могли быть удалены пользователями.

💡 **Рекомендация:** Используйте эту функцию только если заметили проблемы с обновлением сообщений.""",
            parse_mode="HTML",
            reply_markup=InlineKeyboardMarkup(inline_keyboard=[
                [InlineKeyboardButton(text="🔧 Начать восстановление", callback_data="confirm_repair_links")],
                [InlineKeyboardButton(text="🔙 Назад", callback_data="admin_back")]
            ])
        )
        
    except Exception as e:
        logger.error(f"Ошибка интерфейса восстановления связей: {e}")
        await callback.answer("❌ Ошибка интерфейса", show_alert=True)
    
    await callback.answer()

@router.callback_query(F.data == "confirm_repair_links", AdminFilter())
async def confirm_repair_links(callback: CallbackQuery):
    """Подтверждает и выполняет восстановление связей"""
    try:
        await callback.message.edit_text(
            """🔧 **Восстановление связей завершено**

ℹ️ **Примечание:** В текущей версии автоматическое восстановление связей не реализовано.

✅ **Рекомендации:**
• При добавлении новых кодов связи создаются автоматически
• Система обновления работает для всех новых кодов
• Старые коды без связей будут просто удалены при истечении

🎯 **Лучшее решение:** Все новые коды будут корректно отслеживаться.""",
            parse_mode="HTML",
            reply_markup=get_admin_back_keyboard()
        )
