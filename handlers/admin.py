"""
Исправленный admin.py с правильным порядком обновления сообщений
"""
import asyncio
import logging
import os
...
• Сохранено связей: {stats.get('saved_links', 0)}""",
                parse_mode="HTML"
            )
...
async def confirm_expire_code(callback: CallbackQuery, bot: Bot):
    """КРИТИЧНО: Правильный порядок деактивации с обновлением сообщений"""
    code = callback.data.replace("confirm_expire_", "")
    
    try:
        logger.info(f"🔄 Начинаю деактивацию кода: {code}")
        
        # ШАГ 1: Обновляем сообщения ПЕРЕД удалением кода
        logger.info(f"📝 Обновляю сообщения для кода: {code}")
        update_result = await update_expired_code_messages(bot, code)
        
        # ШАГ 2: Удаляем код из базы данных ПОСЛЕ обновления сообщений
        logger.info(f"🗑️ Удаляю код из БД: {code}")
...
🔄 <b>Сообщений обновлено:</b> {update_result.get('updated', 0)}
❌ <b>Ошибок обновления:</b> {update_result.get('failed', 0)}
📊 <b>Статус:</b> Полностью удален из базы данных

💡 <i>Все старые сообщения пользователей теперь показывают, что код истек</i>""",
                parse_mode="HTML",
                reply_markup=get_admin_back_keyboard()
            )
